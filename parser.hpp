...
int parseDefectMetadata(parameter * & params, const int numparam, defects_metadata & defects_sim, metadata & sim)
{
	...
	defects_sim.phifile[0] = '\0';
	defects_sim.pifile[0] = '\0';
	...
	{
	COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": The defect flag is not set properly!" << endl;
#ifdef LATFIELD2_HPP
	    parallel.abortForce();
#endif
	}
	...
	if (defects_sim.defect_flag==DEFECT_GLOBAL)
	{
		...
		if (parseParameter(params, numparam, "defect ic", par_string))
		{
			if (par_string[0] == 'P' || par_string[0] == 'p')
				defects_sim.defect_ic = DEFECT_IC_PREVOLUTION;
			else if (par_string[0] == 'R' || par_string[0] == 'r')
				defects_sim.defect_ic = DEFECT_IC_READ_FROM_FILE;
			else
			{
				COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": Defect IC generator not recognized!" << endl;
	#ifdef LATFIELD2_HPP
				parallel.abortForce();
	#endif
			}
		}
	// parsing the defect IC params
		if(defects_sim.defect_ic == DEFECT_IC_READ_FROM_FILE)
		{
			COUT << "Defect IC set to read from file" << endl;
			parseParameter(params, numparam, "field file", defects_sim.phifile);
			parseParameter(params, numparam, "field_velocity file", defects_sim.pifile);
			if(!parseParameter(params, numparam, "field file", defects_sim.phifile) || !parseParameter(params, numparam, "field_velocity file", defects_sim.pifile))
			{
				COUT << "The file names are not set properly!" << endl;
	#ifdef LATFIELD2_HPP
				parallel.abortForce();
	#endif
			}
		}
		else if(defects_sim.defect_ic == DEFECT_IC_PREVOLUTION)
		{
			COUT << "Defect IC set to prevolution" << endl;
		...
		parseParameter(params, numparam, "eta2_scaling", defects_sim.eta2_scaling);
		if (defects_sim.eta2_scaling <= 0 || !isfinite(defects_sim.eta2_scaling))
		{
			COUT << COLORTEXT_RED << " error" << COLORTEXT_RESET << ": the eta square for the scalar field in global defects not set properly!" << endl;
	#ifdef LATFIELD2_HPP
			parallel.abortForce();
		...
		parseParameter(params, numparam, "lambda0", defects_sim.lambda0);
		if (defects_sim.lambda0 <= 0 || !isfinite(defects_sim.lambda0))
		{
		...
		if (parseParameter(params, numparam, "lambda scaling", par_string))
		{
			if (par_string[0] == 'n' || par_string[0] == 'N')
			{
				defects_sim.s = NO_LAMBDA_SCALING ;
				COUT << endl << COLORTEXT_BLUE << "the lambda scaling is set to: NO" << COLORTEXT_RESET << endl;
			}
			else if (par_string[0] == 'y' || par_string[0] == 'Y')
			{
				defects_sim.s = LAMBDA_SCALING ;
				COUT << endl << COLORTEXT_BLUE << "the lambda scaling is set to: YES" << COLORTEXT_RESET << endl;
			}
		}
		...
		COUT<< "The global defect parameters used are:"<<" "<<"eta square = " << COLORTEXT_BLUE << defects_sim.eta2 << COLORTEXT_RESET << ", eta scaling = " << defects_sim.eta2_scaling << ", lambda0 = " << COLORTEXT_BLUE << defects_sim.lambda0 << COLORTEXT_RESET << ", number of components = " << COLORTEXT_BLUE << defects_sim.nComponents << COLORTEXT_RESET << endl << endl;
